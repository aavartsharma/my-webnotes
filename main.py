...
def show_welcome_page():
    ...
    welcome_label = ttk.Label(root,text="Welcome to notesite",font=custom_font)
    welcome_label.grid(row=0,column=1,padx=10,pady=5)
    # exit button ,new ,search ,edit
    newNote_button = ttk.Button(root, text= "make new note",command= show_UI_new_Project)
    newNote_button.grid(row=1,column=1,padx=10,pady=1)
    root.mainloop()
    pass
...
def show_UI_new_Project():
    ...
    colorbutton = ttk.Button(root, text="Choose Color", command=choose_color)
    ...
    content.grid(row=5,column=1,padx=10,pady=5)


    addLineButton = ttk.Button(root,text="Add",command=h)
    addLineButton.grid(row=7,column=1,padx=10,pady=5)

    submit_button = ttk.Button(root,text="Submit",command=lambda: display_UI(content))
    submit_button.grid(row=8, column=1, padx=10, pady=5)

    close_button = ttk.Button(root, text="Close", command=lambda: close_window(root))
    close_button.grid(row=8, column=2, padx=10, pady=5)
